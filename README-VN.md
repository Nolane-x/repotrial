...
## Vì sao đội ngũ chọn RepoTrial

- **Một hồ sơ thay vì nhiều dashboard:** phân tích tĩnh, evidence runtime, inventory supply-chain, finding chênh lệch và provenance dùng chung định danh xác định.
- **Dùng tốt trước ForgeOS, sâu hơn khi có ForgeOS:** phân tích local/offline đã hoàn chỉnh; ForgeOS bổ sung evidence runtime và khắc phục được neo độc lập.
- **Sinh ra để vào CI:** HTML portable, JSON, SARIF, CycloneDX, proof và chữ ký tùy chọn đến từ cùng một lần quét.

## Luồng bằng chứng

```text
repository → khám phá có giới hạn → rule local / provider tùy chọn → canonical charges
           → verdict theo coverage + định danh differential          → artifact portable
           → proof + provenance + chữ ký tùy chọn                   → CI hoặc human review
```

Provider luôn được bật rõ ràng và mọi thất bại đều được ghi thành trạng thái evidence; không có đường nào âm thầm biến thất bại thành verdict sạch. Xem [kiến trúc](docs/architecture.md) và [runtime sandbox](docs/runtime-sandbox.md) để biết chi tiết.
