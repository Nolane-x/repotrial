  ...
  return (
    ...
    >
      ...
      <nav className="site-nav" aria-label="Primary">
        ...
        <div className="nav-state" aria-hidden="true">
          <span>{String(state.beat.index).padStart(2, '0')}</span>
          <span className="nav-state-title">{state.beat.intent}</span>
        </div>
  ...
  return (
    <section id={isHero ? 'top' : beat.id} data-beat={beat.id} className={`beat beat--${beat.id}`}>
      <div className="beat-inner">
        <motion.div
          className="beat-copy"
          initial={reducedMotion ? false : { opacity: 0.18, y: 46 }}
        ...
        >
          ...
          {isHero ? (
            <h1>INTELLIGENCE <em>IS NOT</em> A MODEL.<br /><strong>IT IS A SYSTEM.</strong></h1>
          ) : (
            <h2>{beat.title}</h2>
          )}
...
function ArchitectureRows() {
  return <div className="architecture-rows">{DOMAIN_ROWS.map(([name, detail], index) => (
    <div className="architecture-row" key={name}><span>{String(index + 1).padStart(2, '0')}</span><b>{name}</b><p>{detail}</p><i /></div>
  ))}</div>;
}
